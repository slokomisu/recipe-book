  ...
  renderIngredients = ({fields, meta: {error, submitFailed}}) => {
    return (
        <div>
          ...
          <Button type="button" color="blue" content="Add Ingredient"
                  icon="plus" onClick={() => fields.push({})}/>
        </div>
    );
  };

  render() {
    const {selectedRecipe, imagePath, handleSubmit, editMode} = this.props;
    if (!selectedRecipe && editMode) {
      return <Loading/>;
    } else if (!selectedRecipe && !editMode) {
    ...
    return (
        <Container fluid>
          <Form onSubmit={handleSubmit}>
            ...
            <Divider/>
            <Field component={SemanticUiField} as={Form.Input} name="name"
                   label="Name"/>
            <Field component={SemanticUiField} as={Form.Input} name="imagePath"
                   label="Image URL" type="text"/>
            <Image src={imagePath}/>
            <Field component={SemanticUiField} as={Form.TextArea}
